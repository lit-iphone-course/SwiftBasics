// 10.1 OptionalのInt型(変数名: optionalNumber)を定義してprintしてください

// 10.2 unwrappedNumberがnilの場合に初期値が10になるようにして、printしてください

// 10.3 optionalNumberをif let文を使ってアンラップしてprintしてください

// 10.4 optionalNumberをguard let文を使ってアンラップしてprintしてください

// 10.5 optionalNumberをOptionalのString型(変数名: unwrappedText)を定義してprintしてください


// 10.6 unwrappedTextがnilの場合に初期値が"Hello"になるようにして、printしてください


// 10.7 unwrappedTextをif let文を使ってアンラップしてprintしてください（OptionalのString型に対して）


// 10.8 unwrappedTextをguard let文を使ってアンラップしてprintしてください（OptionalのString型に対して）


// 10.9 OptionalのDouble型(変数名: unwrappedDouble)を定義してprintしてください


// 10.10 変数unwrappedDoubleがnilの場合に初期値が3.14になるようにして、printしてください


// 10.11 変数unwrappedDoubleをif let文を使ってアンラップしてprintしてください


// 10.12 変数unwrappedDoubleをguard let文を使ってアンラップしてprintしてください


// 10.13 配列の要素をアンラップしてprintしてください
let numbers: [Int?] = [1, nil, 3, nil, 5]


// 10.14 if let文を使って配列の要素をアンラップしてprintしてください


// 10.15 guard let文を使って配列の要素をアンラップしてprintしてください


// 10.16 ??を使って配列の要素がnilの場合に初期値を指定してprintしてください

